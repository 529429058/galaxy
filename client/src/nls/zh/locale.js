...
define({
    ...
    "Source for the reference genome": "参考基因组来源",
    "Select a reference genome": "选择参考基因组",
    "Select the reference genome": "选择参考基因组",
    "Is this a single or paired library": "这是一个单一的还是配对的库",
    "FASTA/Q file": "FASTA/Q 文件",
    "Specify strand information": "指定链信息",
    "FASTA/Q file #1": "FASTA/Q 文件 #1",
    "FASTA/Q file #2": "FASTA/Q 文件 #2",
    "Paired Collection": "配对集合",
    "Interleaved FASTA/Q file": "交错的 FASTA/Q 文件",
    "Output alignment summary in a more machine-friendly style": "以更机器友好的风格输出对齐摘要",
    "Print alignment summary to a file": "将对齐摘要打印到文件",
    "Input options": "输入选项",
    "Skip the first N reads or pairs in the input": "跳过输入中的前 N 个读取或配对",
    "Align the first N reads or read pairs from the input (after the first N reads or pairs have been skipped), then stop": "对输入中的前 N 个读取或配对进行比对（在跳过前 N 个读取或配对之后），然后停止",
    "Trim 5": "修剪 5' 端",
    "Trim 3": "修剪 3' 端",
    "Select quality score encoding": "选择质量分数编码",
    "Convert input qualities from Solexa (which can be negative) to Phred (which can": "将输入质量从 Solexa（可能是负数）转换为 Phred（可以",
    "Are quality values provided as space separated integers?": "是否提供以空格分隔的整数质量值？",
    "Alignment options": "对齐选项",
    "Function governing the maximum number of ambiguous characters (usually Ns and/or .s) allowed in a read as a function of read length": "控制读取中允许的模糊字符（通常是 N 或 .）最大数量的函数，作为读取长度的函数",
    "Constant term (B)": "常数项 (B)",
    "Coefficient (A)": "系数 (A)",
    "Ignore quality values": "忽略质量值",
    "Skip forward strand of reference": "跳过参考的正向链",
    "Skip reverse strand of reference": "跳过参考的反向链",
    ...
    "Spliced alignment options": "剪接对齐选项",
    ...
    "Disable spliced alignment": "禁用剪接对齐",
    "Minimum fragment length for valid paired-end alignments": "有效配对末端对齐的最小片段长度",
    "Maximum fragment length for valid paired-end alignments": "有效配对末端对齐的最大片段长度",
    ...
    "Primary alignments": "主要对齐",
    "Output options": "输出选项",
    "Write unaligned reads (in fastq format) to separate file(s)": "将未对齐的读取（以 fastq 格式）写入单独的文件",
    "Write aligned reads (in fastq format) to separate file(s)": "将对齐的读取（以 fastq 格式）写入单独的文件",
    "SAM options": "SAM 选项",
