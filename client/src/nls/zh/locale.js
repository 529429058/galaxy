...
define({
    ...
    "Public Name or Email Address": "账号名称或邮箱地址",
    ...
    "Username": "用户名",
    "Your Dropbox Account": "您的 Dropbox 帐户",
    "Dropbox access token": "Dropbox 访问令牌",
    "Your B2DROP Account": "您的 B2DROP 帐户",
...
// hisat2 auto translated
    "Summary Options": "摘要选项",
    "Advanced Options": "高级选项",
    "Read Groups text": "读取组文本",
    "Source for the reference genome": "参考基因组的来源",
    "Select a reference genome": "选择一个参考基因组",
    "Select the reference genome": "选择参考基因组",
    "Is this a single or paired library": "这是一个单端还是双端文库？",
    "FASTA/Q file": "FASTA/Q 文件",
    "Specify strand information": "指定链信息",
    "FASTA/Q file #1": "第一个 FASTA/Q 文件",
    "FASTA/Q file #2": "第二个 FASTA/Q 文件",
    "Paired Collection": "配对集合",
    "Interleaved FASTA/Q file": "交错的 FASTA/Q 文件",
    "Output alignment summary in a more machine-friendly style.": "以更机器友好的风格输出比对摘要。",
    "Print alignment summary to a file.": "将比对摘要打印到文件。",
    "Input options": "输入选项",
    "Skip the first N reads or pairs in the input": "跳过输入中的前 N 个读取或配对",
    "Align the first N reads or read pairs from the input (after the first N reads or pairs have been skipped), then stop": "比对输入中的前 N 个读取或配对（跳过前 N 个读取或配对之后），然后停止",
    "Trim 5": "修剪 5' 端",
    "Trim 3": "修剪 3' 端",
    "Select quality score encoding": "选择质量分数编码方式",
    "Convert input qualities from Solexa (which can be negative) to Phred (which cannot be negative)": "将输入质量从 Solexa（可以是负数）转换为 Phred（不能是负数）",
    "Are quality values provided as space separated integers?": "是否提供以空格分隔的整数质量值？",
    "Alignment options": "比对选项",
    "Function governing the maximum number of ambiguous characters (usually Ns and/or .s) allowed in a read as a function of read length": "控制读取中允许的最大模糊字符数（通常是 N 或 .）的函数，作为读取长度的函数",
    "Constant term (B)": "常数项 (B)",
    "Coefficient (A)": "系数 (A)",
    "Ignore quality values": "忽略质量值",
    "Skip forward strand of reference": "跳过参考序列的正链",
    "Skip reverse strand of reference": "跳过参考序列的负链",
    "Scoring options": "评分选项",
    "Maximum mismatch penalty": "最大错配惩罚",
    "Minimum mismatch penalty": "最小错配惩罚",
    "Maximum soft-clipping penalty": "最大软剪切惩罚",
    "Minimum soft-clipping penalty": "最小软剪切惩罚",
    "Allow soft-clipping": "允许软剪切",
    "Ambiguous read penalty": "模糊读取惩罚",
    "Read gap open penalty": "读取间隙开放惩罚",
    "Read gap extend penalty": "读取间隙延伸惩罚",
    "Reference gap open penalty": "参考间隙开放惩罚",
    "Reference gap extend penalty": "参考间隙延伸惩罚",
    "Function governing the minimum alignment score needed for an alignment to be considered \"valid\" (i.e. good enough to report)": "控制一个比对被认为是“有效”的所需的最小比对分数的函数（即足够好以报告）",
    "Spliced alignment options": "剪接比对选项",
    "Penalty for canonical splice sites": "标准剪接位点的惩罚",
    "Penalty for non-canonical splice sites": "非标准剪接位点的惩罚",
    "Penalty function for long introns with canonical splice sites": "具有标准剪接位点的长内含子的惩罚函数",
    "Penalty function for long introns with non-canonical splice sites": "具有非标准剪接位点的长内含子的惩罚函数",
    "Minimum intron length": "最小内含子长度",
    "Maximum intron length": "最大内含子长度",
    "Disable spliced alignment": "禁用剪接比对",
    "Minimum fragment length for valid paired-end alignments": "有效配对末端比对的最小片段长度",
    "Maximum fragment length for valid paired-end alignments": "有效配对末端比对的最大片段长度",
    "GTF file with known splice sites": "包含已知剪接位点的 GTF 文件",
    "Transcriptome assembly reporting": "转录组组装报告",
    "Disable automatic template length adjustment for RNA-seq reads": "禁用 RNA-seq 读取的自动模板长度调整",
    "reports a list of novel splice sites": "报告新剪接位点的列表",
    "Reporting options": "报告选项",
    "Primary alignments": "主要比对",
    "Output options": "输出选项",
    "Write unaligned reads (in fastq format) to separate file(s)": "将未比对的读取（以 fastq 格式）写入单独的文件",
    "Write aligned reads (in fastq format) to separate file(s)": "将比对的读取（以 fastq 格式）写入单独的文件",
    "SAM options": "SAM 选项",
    "Suppress SAM records for reads that failed to align.": "抑制未成功比对的读取的 SAM 记录",
    "Edit Read Group IDs": "编辑读取组 ID",
    "Read Group ID": "读取组 ID",
    "Read Group text": "读取组文本",
    "Add/remove": "添加/移除",
    "When printing secondary alignments, HISAT2 by default will write out the SEQ and QUAL strings. Specifying this option causes HISAT2 to print an asterisk in those fields instead.": "在打印次要比对时，HISAT2 默认会写出 SEQ 和 QUAL 字符串。指定此选项会导致 HISAT2 在这些字段中打印一个星号。",
    "Other options": "其他选项",
    "Use this number as the seed for pseudo-random number generator": "使用此数字作为伪随机数生成器的种子",
    "Re-initialize the pseudo-random generator for each read using the current time": "使用当前时间重新初始化每个读取的伪随机生成器",
});
